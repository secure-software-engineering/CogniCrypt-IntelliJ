...
public class CogniCryptErrorLineMarker implements LineMarkerProvider
{
    ...
    public LineMarkerInfo getLineMarkerInfo(@NotNull PsiElement psiElement)
    {
        if (psiElement instanceof PsiStatement)
        {
            ...
            if (!errors.isEmpty()) return CreateNewMarker(psiElement, errors);
        }
        return null;
    }
    ...
    {
        return new LineMarkerInfo<>(psiElement, psiElement.getTextRange(), PluginIcons.ERROR, Pass.EXTERNAL_TOOLS, new TooltipProvider(getErrorsMessage(errors)), null, GutterIconRenderer.Alignment.LEFT);
    }
